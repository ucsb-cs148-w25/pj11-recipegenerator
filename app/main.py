...
from app.ML_functions import generate_delicious_recipes
...
def generate_recipes():
    """
    Endpoint to generate recipe suggestions based on current fridge contents.
    Returns three recipe suggestions using the items currently in the fridge.
    
    Returns:
        dict: Contains recipe suggestions generated by the ML model
    
    Raises:
        HTTPException: If the fridge is empty
    """
    # Get all items from the fridge
    items = fridge_items.find()
    fridge_contents = [(item["name"], item["quantity"]) for item in items]
    ...
    try:
        # Generate recipes using the ML function
        recipes = generate_delicious_recipes(fridge_contents)
